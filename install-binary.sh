...
PROJECT_NAME="helm-init"
PROJECT_GH="billyshambrook/$PROJECT_NAME"
...
getDownloadURL() {
  ...
  if [ -n "$version" ]; then
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/download/$version/$PROJECT_NAME-$OS.tgz"
  else
...
installFile() {
  ...
  HELM_TMP_BIN="$HELM_TMP/init/bin/init"
